...
inline void make_nonblocking(int fd) {
  if (fcntl(fd, F_SETFL, fcntl(fd, F_GETFL) | O_NONBLOCK) != 0) {
    ROMULUS_FATAL("fcntl(): {}", strerror(errno));
  }
}
...
// Credit: Mu LPD-EPFL
// (https://github.com/LPD-EPFL/mu/blob/master/ctrl/src/device.cpp)
//
inline std::string link_layer_str(uint8_t link_layer) {
  switch (link_layer) {
  ...
  for (int i = 1; i <= dev_attr.phys_port_cnt; ++i) {
    ...
    if (port_attr.state == IBV_PORT_ACTIVE) {
      if (port_attr.link_layer == IBV_LINK_LAYER_INFINIBAND) {
        ...
        ROMULUS_INFO("Ignoring non-Infiniband port {}: {}", i,
                     link_layer_str(port_attr.link_layer));
      }
    }
  }
  ROMULUS_ASSERT(!ports.empty(), "No active InfiniBand ports found.");
  return ports;
}
...
inline rdma_cm_id *make_listen_id(const std::string &address, uint16_t port) {
  ...
  if (devices.empty()) {
    ROMULUS_FATAL("CreateListeningEndpoint :: no RDMA-capable devices found");
  }
  ...
  if (gai_ret != 0) {
    ROMULUS_FATAL("rdma_getaddrinfo(): {}", gai_strerror(gai_ret));
  }

  ROMULUS_ASSERT(resolved != nullptr, "Did not find an appropriate RNIC");
  ...
  if (err != 0) {
    ROMULUS_FATAL("listener rdma_create_ep():{} for {}:{}", strerror(errno),
                  address, port);
  }
  return listen_id;
}
