...
class BumpAllocator {
 ...
 public:
  ...
  uintptr_t try_allocate_global(
    ...
    ROMULUS_FATAL("Out of memory");  // This is actually unreachable
  }
...
class ComputeThread {
 protected:
  ...
  class op_counter_t {
   ...
   public:
    ...
    op_counter_t(ComputeThread *ct)
        : ct_(ct),
          idx_(0),
          op_counter_num_(ct_->args_->uget(romulus::CN_OPS_PER_THREAD)) {
      auto result = ring_counter_t::acquire(
          ct_->op_counter_end, ct_->op_counter_assignments, op_counter_num_);
      ROMULUS_ASSERT(result.has_value(), "op_counter is not available");
      idx_ = result.value();
      ///       ROMULUS_DEBUG("Debug: op_counter_t idx = {}", idx_);
    }
  ...
  struct staging_buf_t {
    ...
    uint8_t *val() {
      ROMULUS_ASSERT(buf_, "staging buf is not enough");
      ROMULUS_ASSERT(
  ...
  struct seq_staging_buf_t {
    ...
    /// @return A pointer to the staging buffer
    uint8_t *val() {
      ROMULUS_ASSERT(buf_, "seq staging buf is not enough");
      ROMULUS_ASSERT(
          buf_ >= ct_->staging_buf_ &&
              buf_ + size_ <= ct_->staging_buf_ + ct_->staging_buf_size_,
          "Staging buf out of range");
  ...
  struct Lane {
    ...
    Lane(const uint32_t lane_idx,
      ...
      if (lane_op_counters[lane_idx].fetch_add((uint64_t)1) + 1 >=
          romulus::internal::kMaxWr) {
        ROMULUS_FATAL(
            "lane_op_counters[{}] is greater than kMaxWr = {}, please "
            "increase kMaxWr",
            lane_idx, romulus::internal::kMaxWr);
      }
    }
 ...
 public:
  ...
  explicit ComputeThread(uint64_t id, std::shared_ptr<ComputeNode> cn,
    ...
    ROMULUS_INFO("Created thread #{}", id_);
  ...
  ~ComputeThread() {
    ...
    ROMULUS_ASSERT(no_leak_detected(), "Leak detected");
    ROMULUS_INFO("ComputeThread {} shutdown", id_);
  }
  ...
  template <typename T>
  T Read(rdma_ptr<T> ptr, T *seg, bool fence = true, size_t size = sizeof(T)) {
    ...
    send_wr.wr_id = (uint64_t)op_counter;
    send_wr.num_sge = 1;
    send_wr.sg_list = &sge;
    send_wr.opcode = IBV_WR_RDMA_READ;
    send_wr.send_flags = 0;

    if(&signal) send_wr.send_flags |= IBV_SEND_SIGNALED;
    if(fence) send_wr.send_flags |= IBV_SEND_FENCE;
    if(size <= max_inline_data_) send_wr.send_flags |= IBV_SEND_INLINE;
  ...
  T *local_allocate(std::size_t num_elements = 1) {
    ...
    ROMULUS_ASSERT(success, "Failed to allocate memory");
  ...
  T *New(std::size_t n = 1) {
    ...
    ROMULUS_ASSERT(ptr != nullptr, "Failed to allocate memory");
  ...
  template <typename T>
  void Delete(T *ptr) {
    ROMULUS_ASSERT(ptr != nullptr, "Pointer is nullptr");
  ...
  bool inline no_leak_detected() {
    ...
    ROMULUS_ASSERT(op_counter_start == op_counter_end,
    ...
    // ROMULUS_ASSERT(staging_buf_allocations_.empty(),
    ...
    ROMULUS_ASSERT(
        cached_buf_start == cached_buf_end,
 ...
 protected:
  ...
  inline void get_seq_op_result(uint32_t seq_idx, uint32_t coro_idx,
    ...
    for (size_t i = 0; i < seq_send_wrs[coro_idx][seq_idx].staging_bufs.size();
      ...
      ROMULUS_DEBUG("release {} from seq_send_wrs[{}][{}]",
  ...
  inline uint32_t find_seq_idx(rdma_ptr<T> ptr, uint32_t coro_idx) {
    ...
    if (!seq_send_wrs[coro_idx].empty()) {
      ...
      ROMULUS_DEBUG("Debug: seq_send_wrs[{}] size = {}", coro_idx,
      ...
      if (it != seq_send_wrs[coro_idx].end() && !it->second.posted) {
        ROMULUS_ASSERT(it->second.send_wrs.size() < args_->uget(CN_WRS_PER_SEQ),
                       "seq_send_wrs[{}] is full, increase the number of "
                       "seq_send_wrs",
                       it->first);
        seq_idx = last_seq_idx;
        ROMULUS_DEBUG("seq_idx = last_seq_idx = {} is found and not posted",
                      last_seq_idx);
        return seq_idx;
      }
    }
    ...
    // seq_idx
    ROMULUS_ASSERT(
        seq_send_wrs[coro_idx].size() < args_->uget(CN_OPS_PER_THREAD),
    ...
    ROMULUS_DEBUG(
        "seq_send_wrs is empty, add a new seq_idx = {}, lane_idx = {}", seq_idx,
        seq_send_wrs[coro_idx][seq_idx].lane->lane_idx);
    return seq_idx;
  }
};
}  // namespace romulus
