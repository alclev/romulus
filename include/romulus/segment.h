...
inline std::optional<uintptr_t> find_mmap_location(uintptr_t min_addr,
  ...
  if (!(len && !(len & (len - 1)))) {
    ROMULUS_FATAL("len is not a power of 2");
  }
...
class Segment {
  ...
  static int GetNumHugePages() {
    ...
    if (!file.is_open()) {
      ROMULUS_DEBUG("Failed to open file: {}", HUGE_PAGE_PATH);
      return 0;
    }
    int nr_huge_pages;
    file >> nr_huge_pages;
    if (file.fail()) {
      ROMULUS_DEBUG("Failed to read nr_huge_pages");
      return 0;
    }
 ...
 public:
  ...
  Segment(uint64_t cap) : capacity_(cap) {
    ...
    ROMULUS_ASSERT(((void *)raw_) != MAP_FAILED, "mmap failed.");
  }
  ...
  ibv_mr_ptr registerWithPd(ibv_pd *pd) {
    if (pd == nullptr) {
      ROMULUS_FATAL("Cannot register segment with null PD");
    }
    ...
    if (ptr == nullptr) {
      ROMULUS_FATAL("RegisterMemoryRegion :: ibv_reg_mr failed: {}",
                    strerror(errno))
    }
    ROMULUS_INFO("  Registered region 0x{:x} (length=0x{:x}) ({} pages)",
