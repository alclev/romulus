...
class MemoryNode {
 ...
 protected:
  ...
  void handle_connections() {
    ROMULUS_INFO("MemoryNode {} listening on {}:{}", self_.id, address_, port_);
    while (remaining_conns_ > 0) {
      ...
      if (int ret = rdma_get_cm_event(listen_channel_, &event); ret != 0) {
        if (errno != EAGAIN) {
          ROMULUS_FATAL("rdma_get_cm_event(): {}", strerror(errno));
        } else {
      ...
      switch (event->event) {
        ...
        case RDMA_CM_EVENT_CONNECT_ERROR:
          ...
          ROMULUS_FATAL("Unexpected signal: {}", rdma_event_str(event->event));

        default:
          // We did not design for other events, so crash if another event
          // arrives
          ROMULUS_FATAL("Not implemented: {}", rdma_event_str(event->event));
      }
    }
  }
  ...
  void on_connect(rdma_cm_id *id, rdma_cm_event *event, ibv_pd *pd) {
    ...
    ROMULUS_ASSERT(event->param.conn.private_data != nullptr,
    ...
    if (machine_id == self_.id) {
      ROMULUS_FATAL("on_connect called for self-connection");
    }
    ...
    ROMULUS_ASSERT(ret == 0, "rdma_set_option(): {}", strerror(errno));
    ...
    ROMULUS_ASSERT(ret == 0, "rdma_create_qp(): {}", strerror(errno));
    ...
    ROMULUS_ASSERT(ret == 0, "rdma_accept(): {}",
    ...
    if (status.t != romulus::Ok) {
      ROMULUS_FATAL("error in mem node: {}", status.message.value());
    }
    remaining_conns_--;
  }
 ...
 public:
  ...
  ~MemoryNode() {
    ...
    ROMULUS_INFO("MemoryNode shutdown");
    sleep(3);
  }
  ...
  explicit MemoryNode(const MachineInfo &self,
                      std::shared_ptr<romulus::ArgMap> args)
      : self_(self),
        conns_(),
        send_seg_(1 << 20),
        total_threads_(args->uget(romulus::CN_THREADS) *
                       (args->uget(romulus::LAST_CN_ID) -
                        args->uget(romulus::FIRST_CN_ID) + 1)) {
    int id = args->uget(romulus::NODE_ID);
    uint64_t num_segs = args->uget(romulus::SEGS_PER_MN);
    uint64_t seg_size_bits = args->uget(romulus::SEG_SIZE);
    ROMULUS_INFO("Node {}: Configuring Memory Node ({} segments at 2^{}B each)",
                 id, num_segs, seg_size_bits);

    // How many non-localhost compute nodes are there?
    int c0 = args->uget(romulus::FIRST_CN_ID);
    int cn = args->uget(romulus::LAST_CN_ID);
    uint32_t cns = cn - c0 + 1;
    if (id >= c0 && id <= cn) cns--;
    uint32_t qlw = args->uget(romulus::QP_LANES);
    remaining_conns_ = cns * qlw;

    // Make the listening endpoint, and register the sending segment with it
    uint16_t port = args->uget(romulus::MN_PORT);
    listen_id_ = romulus::internal::make_listen_id(self.address, port);
    ROMULUS_ASSERT(listen_id_->pd != nullptr,
    ...
    ROMULUS_INFO("Shared Segments:");
    for (auto ri : ris_) {
      ROMULUS_INFO("  0x{:x} (rk=0x{:x})", ri.raddr, ri.rkey);
    }
    ...
    ROMULUS_INFO("Listener thread awaiting {} connections", remaining_conns_);
    ...
    if (rdma_listen(listen_id_, 0) != 0) {
      ROMULUS_FATAL("rdma_listen(): {}", strerror(errno));
    }
  ...
  void init_done() {
    ROMULUS_INFO("Stopping listening thread...");
    runner_.join();
