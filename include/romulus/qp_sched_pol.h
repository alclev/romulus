...
/// @brief A policy for scheduling QPs (Queue Pairs) to interact with
/// MemoryNodes
/// @details
...
class QpSchedPolicy {
 ...
 public:
  ...
  static Policy to_policy(std::string policy) {
    ...
    ROMULUS_FATAL("Invalid QpSchedPolicy {}", policy);
  }
 ...
 public:
  ...
  QpSchedPolicy(std::shared_ptr<romulus::ArgMap> args)
      : policy_(NONE),
        num_lanes_(args->uget(QP_LANES)),
        num_threads_(args->uget(CN_THREADS)),
        last_lane_(0) {
    // Initialize the per_mn_ counters, so we can switch freely among policies
    for (uint64_t i = 0; i <= args->uget(romulus::LAST_MN_ID); ++i) {
  ...
  void set_policy(Policy policy, uint64_t thread_id) {
    ...
    if (policy_ == ONE_TO_ONE) {
      ROMULUS_ASSERT(num_lanes_ >= num_threads_,
      ...
      ROMULUS_FATAL("Unrecognized QpSchedPol {}", (uint32_t)policy_);
    }
  }
