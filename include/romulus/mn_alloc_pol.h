...
class MnAllocPolicy {
 ...
 public:
  ...
  static Policy to_policy(std::string policy) {
    ...
    ROMULUS_FATAL("Invalid MnAllocPolicy {}", policy);
  }
 ...
 public:
  ...
  MnAllocPolicy(std::shared_ptr<romulus::ArgMap> args)
      : policy_(NONE),
        num_segs_(args->uget(SEGS_PER_MN)),
        // TODO: This assumes MemoryNode Ids start at 0
        num_mns_(args->uget(LAST_MN_ID) + 1),
        total_segs_(num_segs_ * num_mns_),
        last_mn_(0),
        last_seg_(0) {}
  ...
  void set_policy(Policy policy, std::shared_ptr<ArgMap> args,
    ...
    if (policy_ == GLOBAL_MOD) {
      ...
      ROMULUS_FATAL("Unrecognized MnAllocPol {}", (uint32_t)policy_);
    }
  }
