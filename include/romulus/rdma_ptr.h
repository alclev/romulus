...
template <typename T>
inline std::string format_rdma_ptr(const romulus::rdma_ptr<T> &input) {
...
template <typename T>
struct std::hash<romulus::rdma_ptr<T>> {
  std::size_t operator()(const romulus::rdma_ptr<T> &ptr) const {
