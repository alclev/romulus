...
// The maximum inline data size for the device
inline uint32_t max_inline_ = 0;
...
inline rdma_cm_id *initialize_ep(std::string_view address, uint16_t port) {
  ...
  if (int err =
          rdma_getaddrinfo(address.data(), port_str.c_str(), &hints, &resolved);
      err != 0) {
    ROMULUS_FATAL("rdma_getaddrinfo(): {}", gai_strerror(err));
  }
  ...
  if (err) {
    ROMULUS_DEBUG("Address {} port {} seen.", address, port_str);
    ROMULUS_FATAL("compute node rdma_create_ep(): {}", strerror(errno));
  }
  // Must be set after rdma_create_ep() completes successfully
  max_inline_ = init_attr.cap.max_inline_data;
  return id;
}
...
inline Connection *connect_remote(uint32_t my_id, uint32_t mn_id,
  ...
  while (true) {
    ...
    if (rdma_migrate_id(id, event_channel) != 0) {
      ROMULUS_FATAL("rdma_migrate_id(): {}", strerror(errno));
    }
    ...
    if (rdma_set_option(id, RDMA_OPTION_ID, RDMA_OPTION_ID_ACK_TIMEOUT,
      ...
      ROMULUS_FATAL("rdma_set_option(): {}", strerror(errno));
    }
    ...
    if (rdma_connect(id, &conn_param) != 0) {
      ROMULUS_FATAL("rdma_connect(): {}", strerror(errno));
    }
    ...
    while (true) {
      ...
      if (rdma_ack_cm_event(event) != 0) {
        ROMULUS_FATAL("rdma_ack_cm_event(): {}", strerror(errno));
      }
      ...
      // Otherwise fail
        ...
        ROMULUS_FATAL("Got unexpected event: {}", rdma_event_str(cm_event));
      }
    }
  }
}
...
inline Connection *connect_loopback(uint32_t my_id, std::string_view address,
  ...
  if (ibv_query_device(id->verbs, &dev_attr) != 0) {
    ROMULUS_FATAL("ibv_query_device(): {}", strerror(errno));
  }
  ...
  for (int i = 1; i <= dev_attr.phys_port_cnt; ++i) {
    if (ibv_query_port(id->verbs, i, &port_attr) != 0) {
      ROMULUS_FATAL("ibv_query_port(): {}", strerror(errno));
    }
    if (port_attr.state == IBV_PORT_ACTIVE) {
      LOOPBACK_PORT_NUM = i;
      // ROMULUS_DEBUG("Using physical port {} for loopback", i);
      break;
    }
  }
  ...
  if (ibv_modify_qp(id->qp, &attr, attr_mask) != 0) {
    ROMULUS_FATAL("ibv_modify_qp(): {}", strerror(errno));
  }
...
/// one, then chops it up for its ComputeThreads.  This lets romulus only make
/// one registration per connection, while still supporting many threads.
class ComputeNode {
  ...
  // WITH:
  // rkeys_[node_id][segment_id] = rkey
  std::vector<std::vector<uint32_t>> rkeys_;

  const uint64_t seg_mask_;  // The bitmask for finding a segment id
  const uint64_t mask_low_;
  ...
  std::shared_ptr<romulus::ArgMap> args_;  // The program's command-line args
  ...
  void save_region(uint16_t node_id, uint64_t region, uint32_t rkey) {
    ...
    ROMULUS_ASSERT((region & node_mask) == 0, "Top bits of region must be 0");
    // TODO:  Should we also assert that the low bits are 0?  We expect
    //        alignment, after all...
    ROMULUS_ASSERT((region & seg_mask_) == 0,
                   "Region is not aligned to segment size");
    // Check if the seg_id exists at this node_id
    uint64_t seg_id = (region & mask_low_) >> args_->uget(romulus::SEG_SIZE);
    // If the seg_id hasn't already been placed
    if (rkeys_[node_id].size() <= seg_id) {
      rkeys_[node_id].resize(seg_id + 1, 0);
      rkeys_[node_id][seg_id] = rkey;
 ...
 public:
  ...
  uint32_t get_rkey(uint64_t raw) {
    uint64_t node_id = raw >> 48 & 0xFFFF;
    uint64_t seg_id = raw & seg_mask_;
    return rkeys_[node_id][seg_id];
  }
  /// Construct a ComputeNode
  ///
  /// @param self This process's machine Id and DNS address
  /// @param args The command-line arguments to the program
  ComputeNode(const MachineInfo &self, std::shared_ptr<romulus::ArgMap> args)
      : self_(self),
        num_threads_(args->uget(romulus::CN_THREADS)),
        thread_bufsz_(1ULL << args->uget(romulus::CN_THREAD_BUFSZ)),
        seg_(
            (1ULL << (64 - __builtin_clzll(num_threads_ * thread_bufsz_ - 1)))),
        threads_(0),
        seg_mask_((1ULL << args->uget(romulus::SEG_SIZE)) - 1),
        mask_low_((-1ULL) ^ seg_mask_),
        args_(args),
        lane_op_counters_(args->uget(romulus::QP_LANES)) {
    ROMULUS_INFO("Node {}: Configuring Compute Node",
                 args->uget(romulus::NODE_ID));
  ...
  void connect_local(std::vector<MachineInfo> &memnodes,
    ...
    uint64_t qp_lanes = args_->uget(romulus::QP_LANES);
    uint32_t port = args_->uget(romulus::MN_PORT);

    for (auto &p : memnodes) {
      if (p.id == self_.id) {
        for (uint64_t i = 0; i < qp_lanes; ++i) {
          // Connect, then register the big segment with that connection
          ROMULUS_INFO("Connecting to localhost {}:{} (id = {})", p.address,
                       port, p.id);
  ...
  void connect_remote(std::vector<MachineInfo> &memnodes) {
    ...
    for (const auto &p : memnodes) {
      if (p.id != self_.id) {
        for (uint64_t i = 0; i < qp_lanes; ++i) {
          // Connect, then register the big segment with that connection
          ROMULUS_INFO("Connecting to remote machine {}:{} (id = {}) from {}",
          ...
          if (got.status.t != romulus::Ok) {
            ROMULUS_FATAL("{}", got.status.message.value());
          }

          // Save the connection and the regions
          auto lkey = mrs_.back()->lkey;
          save_conn(p.id, conn, lkey);
          for (auto &r : got.val.value()) save_region(p.id, r.raddr, r.rkey);
        }
      }
    }
  }
