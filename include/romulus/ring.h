...
struct ring_counter_t {
  ...
  static inline void release(const size_t idx, size_t &counter_start,
                             ...
                             const size_t counter_num) {
    ROMULUS_ASSERT(counter_assignments[idx] == State::IN_USE,
...
struct ring_buf_t {
  ...
  static inline void release(
    ...
    ROMULUS_ASSERT(
        ring_buf_allocations.find(buf) != ring_buf_allocations.end() &&
    ...
    while (ring_buf_allocations.find(ring_buf_start) !=
      ...
      if (ring_buf_start == ring_buf) {
        ROMULUS_ASSERT(ring_buf + ring_buf_size - to_erase >= 0,
                       "ring_buf_start = ring_buf_ + ring buf size = "
                       "{} is not greater than to_erase = {}",
                       (uint64_t)(ring_buf + ring_buf_size),
                       (uint64_t)to_erase);
      } else {
        ROMULUS_ASSERT(ring_buf_start - to_erase >= 0,
