...
class Connection {
  ...
  romulus::Status SendMessage(T &&msg, Segment &seg, ibv_mr *mr) {
    ...
    if (ibv_post_send(id_->qp, &wr, &bad_wr) != 0) {
      ...
      romulus::Status err = {romulus::InternalError, ""};
  ...
  /// @param seg TODO
  /// @return TODO
  romulus::StatusVal<std::vector<uint8_t>> TryDeliverMessage(Segment &seg) {
    ibv_wc wc;
    ...
    if (ret < 0 && errno != EAGAIN) {
      ...
      return {{romulus::Unavailable, "Retry"}, {}};
    } else {
      switch (wc.status) {
        case IBV_WC_WR_FLUSH_ERR:
          return {{romulus::Aborted, "QP in error state"}, {}};
        case IBV_WC_SUCCESS: {
          // Prepare the response.
          std::span<uint8_t> recv_span{seg.raw(), wc.byte_len};
          romulus::StatusVal<std::vector<uint8_t>> res = {
              romulus::Status::Ok(), std::make_optional(std::vector<uint8_t>(
  ...
  /// @return TODO
  template <typename T>
  romulus::StatusVal<std::vector<T>> TryDeliverVec(Segment &seg) {
    romulus::StatusVal<std::vector<uint8_t>> msg_or = TryDeliverMessage(seg);
    if (msg_or.status.t == romulus::Ok) {
      std::vector<T> vec(msg_or.val.value().size() / sizeof(T));
      std::memcpy(vec.data(), msg_or.val.value().data(),
                  msg_or.val.value().size());
      return {romulus::Status::Ok(), vec};
    }
 ...
 public:
  ...
  template <typename T>
    requires std::ranges::contiguous_range<T>
  romulus::Status Send(T &&msg, Segment &seg, ibv_mr *mr) {
  ...
  /// @return TODO
  template <typename T>
  romulus::StatusVal<std::vector<T>> DeliverVec(Segment &seg) {
    auto p = this->TryDeliverVec<T>(seg);
    while (p.status.t == romulus::Unavailable) {
